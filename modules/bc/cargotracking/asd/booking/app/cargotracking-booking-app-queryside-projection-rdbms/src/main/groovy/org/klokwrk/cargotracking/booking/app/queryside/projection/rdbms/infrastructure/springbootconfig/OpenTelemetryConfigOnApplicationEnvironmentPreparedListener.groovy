 ...
 * class is not marked as Spring component, but rather is configured as a listener in the application itself (see the source of <code>BookingQuerySideProjectionRdbmsApplication</code>). Alternatively,
 * we could do the same thing via <code>META-INF/spring.factories</code> file, but we chose explicit listener configuration at the application since we believe it is more obvious.
 */
@CompileStatic
