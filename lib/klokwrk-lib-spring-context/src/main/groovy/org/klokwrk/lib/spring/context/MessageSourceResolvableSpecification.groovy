...
class MessageSourceResolvableSpecification {
   ...
   * <p/>
   * For example, if we are trying to resolve messages for an object that contains property <code>report</code>, and that <code>report</code> contains a nested property <code>titleText</code>,
   * the value of <code>propertyPath</code> should be <code>report.titleText</code>.
   */
  String propertyPath
}
